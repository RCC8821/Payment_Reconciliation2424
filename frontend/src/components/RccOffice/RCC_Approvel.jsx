...
//   const [revisedAmounts, setRevisedAmounts] = useState({}); // Store revised amounts per UID
...
//     return selectedItems.reduce((sum, uid) => {
//       const item = itemsForSelectedBill.find((i) => i.uid === uid);
//       if (!item?.Amount) return sum;
//       const amountStr = String(item.Amount).replace(/,/g, "");
//       return sum + (parseFloat(amountStr) || 0);
//     }, 0);
//   }, [selectedItems, itemsForSelectedBill]);
...
//     if (e.target.checked) {
//       setSelectedItems(itemsForSelectedBill.map((item) => item.uid));
//     } else {
//       setSelectedItems([]);
//     }
//   };
...
//     try {
//       const promises = selectedItems.map((uid) => {
//         const item = itemsForSelectedBill.find((i) => i.uid === uid);
//         const payload = {
//           uid,
...
//                   type="checkbox"
//                   checked={
//                     selectedItems.length === itemsForSelectedBill.length &&
//                     itemsForSelectedBill.length > 0
//                   }
//                   onChange={handleSelectAll}
//                   className="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500 bg-gray-800 border-gray-600"
//                 />
...
//                         selectedItems.includes(item.uid) ? "bg-indigo-950/60" : ""
//                       }`}
//                     >
...
//                           className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-lg text-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
//                         />
//                       </td>
//                       <td className="px-5 py-4 text-sm text-gray-300">
//                         {item.APPROVAL_DOER || "-"}
...
//                 <p className="text-sm text-gray-300 font-semibold mb-2">Selected Items:</p>
//                 <div className="space-y-1 max-h-40 overflow-y-auto">
//                   {selectedItems.map((uid) => {
//                     const item = itemsForSelectedBill.find((i) => i.uid === uid);
//                     return (
//                       <div key={uid} className="text-xs text-gray-400 flex justify-between">
//                         <span>{item?.ITEM_NAME_1 || item?.uid}</span>
//                         <span className="text-emerald-400">
//                           {revisedAmounts[uid] ? `₹${formatAmount(revisedAmounts[uid])}` : `₹${formatAmount(item?.Amount)}`}
//                         </span>
//                       </div>
//                     );
//                   })}
