...
//   const [updateActualBankIn, { isLoading: isUpdating }] =
//     useUpdateActualBankInMutation();
...
//             Unable to fetch pending payments. Please try again.
//           </p>
//           <button
//             onClick={refetch}
...
//                   <tr>
//                     <th className="px-3 py-3 text-xs font-bold uppercase tracking-wider whitespace-nowrap text-center">
//                       UID
//                     </th>
//                     <th className="px-3 py-3 text-xs font-bold uppercase tracking-wider whitespace-nowrap text-center">
//                       Site Name
...
//                         <span className="inline-block px-2 py-0.5 text-xs bg-blue-100 rounded font-bold text-blue-700">
//                           {row.uid}
//                         </span>
//                       </td>
//                       <td className="px-3 py-2 text-center">
//                         <div
//                           className="max-w-[180px] mx-auto truncate font-medium text-xs"
//                           title={row.SiteName}
//                         >
//                           {row.SiteName}
//                         </div>
...
//                       </td>
//                       <td className="px-3 py-2 text-center">
//                         <span className="inline-block px-2 py-0.5 text-xs bg-green-100 rounded font-bold text-green-700">
//                           {row.NetAmount}
//                         </span>
//                       </td>
//                       <td className="px-3 py-2 text-center">
//                         <div
//                           className="max-w-[180px] mx-auto truncate text-xs"
...
// {/* Table Container - Yeh part replace kar do */}
// <div className="flex-1 overflow-hidden p-3">
//   <div className="bg-white rounded-lg shadow border border-gray-200 h-full overflow-hidden">
//     <div className="h-full overflow-x-auto"> {/* overflow-y removed, sirf x-auto */}
//       <table className="w-full text-sm border-collapse">
//         {/* min-w-[1400px] hata diya - ab table apne content ke hisab se adjust hogi */}

//         <thead className="bg-gradient-to-r from-gray-800 to-gray-900 text-white sticky top-0 z-10">
//           <tr>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">UID</th>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Site</th>
//             <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider">Amount</th>
//             <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider hidden sm:table-cell">CGST</th>
//             <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider hidden sm:table-cell">SGST</th>
//             <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider">Net</th>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider hidden md:table-cell">RCC A/c</th>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider">Mode</th>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider hidden lg:table-cell">Chq No</th>
//             <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider hidden lg:table-cell">Chq Date</th>
//             <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider hidden xl:table-cell">Photo</th>
//             <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider">Planned</th>
...
//               </td>
//               <td className="px-4 py-3">
//                 <div className="max-w-[150px] truncate font-medium" title={row.SiteName}>
//                   {row.SiteName}
//                 </div>
//               </td>
//               <td className="px-4 py-3 text-right font-medium">{row.Amount}</td>
//               <td className="px-4 py-3 text-right hidden sm:table-cell">{row.CGST || "-"}</td>
//               <td className="px-4 py-3 text-right hidden sm:table-cell">{row.SGST || "-"}</td>
//               <td className="px-4 py-3 text-right">
//                 <span className="inline-block px-2 py-1 text-xs bg-green-100 rounded font-bold text-green-700">
//                   {row.NetAmount}
//                 </span>
//               </td>
//               <td className="px-4 py-3 hidden md:table-cell">
//                 <div className="max-w-[160px] truncate" title={row.RccCreditAccountName}>
//                   {row.RccCreditAccountName}
//                 </div>
//               </td>
//               <td className="px-4 py-3">
...
//                     target="_blank"
//                     rel="noopener noreferrer"
//                     className="inline-flex items-center justify-center p-2 bg-blue-50 text-blue-600 rounded hover:bg-blue-600 hover:text-white transition"
//                   >
...
//                     <p className="text-blue-600 text-xs uppercase font-bold mb-1">
//                       Net Amount
//                     </p>
...
//                     <p className="text-purple-600 text-xs uppercase font-bold mb-1">
//                       Site Name
//                     </p>
...
const Actual_Payment_in = () => {
  ...
  if (pendingPayments.length === 0) {
    return (
      <div className="w-full max-w-full min-h-screen bg-gray-50 p-2 md:p-4 overflow-x-hidden box-border flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-3xl font-bold text-gray-700 mb-4">No Pending Entries</h2>
          <p className="text-gray-500 mb-8">All planned payments have actual status filled.</p>
          <button 
            onClick={refetch} 
              ...
              <tbody className="divide-y divide-gray-50">
                {pendingPayments.map((row) => (
                  <tr key={row.uid} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-4 text-[11px] font-medium">{row.Amount}</td>
                    <td className="px-3 py-4 text-[11px]">{row.CGST || "-"}</td>
                    <td className="px-3 py-4 text-[11px]">{row.SGST || "-"}</td>
                    <td className="px-3 py-4 text-[11px]">
                      <span className="inline-block px-2 py-1 text-xs bg-green-100 rounded font-bold text-green-700">
                        {row.NetAmount}
                      </span>
                    </td>
                    <td className="px-3 py-4 text-[11px] break-words whitespace-normal align-top max-w-xs">
                      {row.RccCreditAccountName || "-"}
                    </td>
                    <td className="px-3 py-4 text-[11px]">
                      <span className="inline-block px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded">
                    ...
                    <td className="px-3 py-4 text-[11px]">{row.ChequeNo || "-"}</td>
                    <td className="px-3 py-4 text-[11px] whitespace-nowrap">{row.ChequeDate || "-"}</td>
                    <td className="px-3 py-4 text-center">
                      {row.ChequePhoto ? (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-block p-1.5 bg-blue-50 text-blue-600 rounded hover:bg-blue-600 hover:text-white transition"
                        >
                          <Eye className="w-4 h-4" />
                        </a>
                      ) : (
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-2 z-[999]">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-hidden flex flex-col">
            <div className="bg-gray-800 p-4 flex justify-between items-center text-white">
              <h2 className="text-sm font-bold">Update Actual Bank In</h2>
            ...
            <div className="p-4 overflow-y-auto space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-center">
                ...
                <div className="p-2 bg-blue-50 rounded border border-blue-200">
                  <p className="text-[8px] text-blue-600 uppercase font-bold">Net Amount</p>
                  <p className="text-sm font-black text-blue-700">{selectedRow.NetAmount}</p>
                </div>
                <div className="p-2 bg-purple-50 rounded border border-purple-200">
                  <p className="text-[8px] text-purple-600 uppercase font-bold">Site</p>
                  <p className="text-[10px] font-bold truncate">{selectedRow.SiteName}</p>
                </div>
              </div>
              ...
              {/* <div>
                ...
                <input
                  ...
                  value={status}
                  onChange={(e) => setStatus(e.target.value)}
                  placeholder="e.g., Received, Cleared, Pending"
