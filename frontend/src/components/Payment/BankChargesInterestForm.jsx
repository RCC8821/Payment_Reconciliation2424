...
import {
  ...
  CreditCard,
  Banknote,
  FolderOpen,  // ✅ Added for Project Name icon
} from 'lucide-react';
...
const BankChargesInterestForm = () => {
  ...
  // ✅ Separate states for project names and bank names
  const [projectNames, setProjectNames] = useState([]);
  const [bankNames, setBankNames] = useState([]);
  const [loadingDropdowns, setLoadingDropdowns] = useState(false);

  // Form State - ✅ Added projectName
  const [formData, setFormData] = useState({
    projectName: '',         // ✅ Added
    bankPayment: '',
    chargesInterestDetails: '',
  ...
  // ✅ Fetch both Project Names and Bank Names from API
  useEffect(() => {
    fetchDropdownData();
  }, []);
  ...
  const fetchDropdownData = async () => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/Bank-Interest-Dropdown-Data`);
      if (res.data.success) {
        setProjectNames(res.data.projectNames || []);  // ✅ Set project names
        setBankNames(res.data.accounts || []);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Added projectName to validation
    if (
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // ✅ Reset form including projectName
        setFormData({
          projectName: '',
          bankPayment: '',
          chargesInterestDetails: '',
