...
router.get('/GET-VRN-Office-Expenses-Data-Approved1', async (req, res) => {
  try {
    // Safety check
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: 'VRN_INC_OFFICE!A8:S', // ← Apne sheet ke tab name aur range ke hisaab se badal sakte ho
    ...
    const filteredData = rows
      .filter(row => row[17] && !row[18])   // Pending approval wale only (optional)
      .map(row => ({
    ...
    REMARK_1:      (row[16] || '').toString().trim(),
        PLANNED_2:(row[17] || '').toString().trim(),
        ACTUAL_2:(row[18] || '').toString().trim()
      }));
...
router.post('/update-VRN-OFFICE-Expenses-Data-Approved1', async (req, res) => {
  ...
  try {
    ...
    await sheets.spreadsheets.values.batchUpdate({
      ...
      resource: {
        ...
        data: [
          { range: `VRN_INC_OFFICE!T${sheetRowNumber}`, values: [[STATUS_2 || '']] },
          { range: `VRN_INC_OFFICE!V${sheetRowNumber}`, values: [[REVISED_AMOUNT_3 || '']] },
          { range: `VRN_INC_OFFICE!W${sheetRowNumber}`, values: [[APPROVAL_DOER_2 || '']] },
          { range: `VRN_INC_OFFICE!X${sheetRowNumber}`, values: [[REMARK_2 || '']] }
        ]
      }
    });
