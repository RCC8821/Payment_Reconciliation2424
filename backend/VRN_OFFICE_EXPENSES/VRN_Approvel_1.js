...
router.get('/GET-VRN-Office-Expenses-Data-Approved1', async (req, res) => {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: 'VRN_INC_OFFICE!A8:Q', // ← Apne sheet ke tab name aur range ke hisaab se badal sakte ho
  ...
  try {
    ...
    await sheets.spreadsheets.values.batchUpdate({
      ...
      resource: {
        ...
        data: [
          { range: `VRN_INC_OFFICE!R${sheetRowNumber}`, values: [[STATUS_2 || '']] },
          { range: `VRN_INC_OFFICE!T${sheetRowNumber}`, values: [[REVISED_AMOUNT_3 || '']] },
          { range: `VRN_INC_OFFICE!U${sheetRowNumber}`, values: [[APPROVAL_DOER_2 || '']] },
          { range: `VRN_INC_OFFICE!V${sheetRowNumber}`, values: [[REMARK_2 || '']] }
        ]
      }
    });
