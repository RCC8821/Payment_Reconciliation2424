...
// Helper function to get unique values ignoring case
const getUniqueIgnoreCase = (arr) => {
  ...
  arr.forEach(item => {
    if (item && item.toString().trim()) {
      ...
      if (!map.has(key)) {
        // Store the first occurrence (you can also choose to capitalize it)
        map.set(key, item.toString().trim());
      }
    }
  });
...
const matchesIgnoreCase = (value, filterArray) => {
  ...
  return filterArray.some(f => f.toString().toLowerCase().trim() === valueLower);
};
...
const MultiSelectFilter = ({
  ...
  // Case-insensitive check if option is selected
  const isOptionSelected = (option) => {
    if (option === "All") return isAllSelected;
    return value.some(v => v.toLowerCase().trim() === option.toLowerCase().trim());
  };
...
const Outstanding = ({ isDarkMode }) => {
  ...
  // Filtered data – with case-insensitive matching
  const filteredData = useMemo(() => {
    if (!Array.isArray(rawData)) {
      return [];
    }
    ...
    if (filters.expHeads.length > 0 && !filters.expHeads.includes("All")) {
      data = data.filter((t) => matchesIgnoreCase(t.expHead, filters.expHeads));
    }
    if (filters.billNos.length > 0 && !filters.billNos.includes("All")) {
  ...
  // Filter options – with case-insensitive unique values
  const filterOptions = useMemo(() => {
    if (!Array.isArray(rawData)) {
      return {
        siteNames: ["All"],
    ...
    // Get all unique site names (case-insensitive)
    ...
    if (filters.siteNames.length > 0 && !filters.siteNames.includes("All")) {
      current = current.filter((t) => matchesIgnoreCase(t.siteName, filters.siteNames));
    }
    
    // Get unique vendors from filtered data (case-insensitive)
    ...
    if (filters.vendorNames.length > 0 && !filters.vendorNames.includes("All")) {
      current = current.filter((t) => matchesIgnoreCase(t.vendorName, filters.vendorNames));
    }
    
    // Get unique expense heads from filtered data (case-insensitive)
    const heads = getUniqueIgnoreCase(
      current.map((t) => t.expHead).filter((h) => h && h !== "—")
    );

    // Filter by selected expense heads
    if (filters.expHeads.length > 0 && !filters.expHeads.includes("All")) {
      current = current.filter((t) => matchesIgnoreCase(t.expHead, filters.expHeads));
    }
    
    // Get unique bill numbers from filtered data (case-insensitive)
...
const Summary = () => {
  ...
  const [filters, setFilters] = useState({
    siteNames: [],
    bankNames: [],
    vendorNames: [], // Added vendorNames filter
    expHeads: [],
  });
    ...
    return transactions
      .map((t) => {
        ...
        return {
          ...
          type: inAmt > 0 ? "in" : "out",
          category: t.expHead || t.siteName || "General",
          vendorName: t.vendorName || t.partyName || "", // Added vendorName
        };
      })
      .filter((t) => t.date !== null && !isNaN(t.date?.getTime()));
  }, [apiData]);

  // All unique options with case-insensitive deduplication
  const allUniqueOptions = useMemo(
    () => ({
  ...
  // Cascading options with case-insensitive filtering
  const cascadingOptions = useMemo(() => {
    let data = processedTransactions;
  ...
  // Updated active filters count to include vendorNames
  const activeFiltersCount =
    filters.siteNames.length +
    filters.bankNames.length +
    filters.vendorNames.length +
          ...
          {activeTab === "summary" && (
            <>
              ...
              <div className="mt-8 space-y-6 relative z-[100]">
                ...
                {/* Updated grid to 4 columns for 4 filters */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                  ...
                  <MultiSelectFilter
                    ...
                    options={cascadingOptions.vendorNames}
                    placeholder="Search & select vendors..."
                    isDarkMode={isDarkMode}
                  />
                  <MultiSelectFilter
            ...
            {hasNoTransactionsInPeriod && (
              ...
              >
                ...
                <button
                  onClick={() => {
                    setPeriod("all");
                    setFilters({ siteNames: [], bankNames: [], vendorNames: [], expHeads: [] });
                  }}
                  className="mt-4 px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors"
                >
                  Reset All
