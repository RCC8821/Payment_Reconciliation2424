...
//         <button
//           onClick={closeModal}
//           className="text-gray-400 hover:text-white p-2 rounded-lg hover:bg-gray-800/80 transition shrink-0 ml-3"
//         >
...
//       <div className="flex-1 overflow-y-auto px-5 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10 space-y-6 sm:space-y-8
//         scrollbar-thin scrollbar-thumb-indigo-600/70 scrollbar-track-gray-950/40">
...
//         {/* Extra padding at bottom so last field isn't hidden under buttons */}
...
//             onClick={closeModal}
//             disabled={isUpdating}
//             className="px-8 py-4 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg font-medium transition text-base sm:text-lg order-2 sm:order-1"
//           >
//             Cancel
//           </button>
...
//             className={`
//               px-8 sm:px-10 py-4 rounded-lg font-medium flex items-center justify-center gap-3
//               min-w-[200px] sm:min-w-[220px] transition-all text-base sm:text-lg
//               order-1 sm:order-2
...
function VRN_Approvel2() {
  ...
  // Real-time theme sync (bina reload ke change ho)
  useEffect(() => {
    const applyTheme = () => {
      const saved = localStorage.getItem("isDarkMode");
    ...
    // Pehli baar apply karo
    applyTheme();

    // Dusre tab ya kahin se localStorage change hone par suno
    ...
    // Extra safety: har 800ms check (kuch browsers mein storage event miss ho jata hai)
  ...
  const openModal = (expense) => {
    ...
    setFormData({
      STATUS_3: "",
      FINAL_AMOUNT_3:
        expense.PLANNED_3 || expense.Total_Amount || expense.Amount || "",
      REMARK_3: "",
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (
      formData.FINAL_AMOUNT_3 !== "" &&
      isNaN(Number(formData.FINAL_AMOUNT_3))
    ) {
      alert("Please enter a valid number for Final Amount");
      return;
    }
    ...
    const payload = {
      ...
      FINAL_AMOUNT_3: formData.FINAL_AMOUNT_3
        ? Number(formData.FINAL_AMOUNT_3)
        : null,
      REMARK_3: formData.REMARK_3.trim() || null,
    };
  ...
  if (isLoading) {
    return (
      ...
      >
        <div className="flex flex-col items-center space-y-5">
          <Loader2
            className={`w-16 h-16 animate-spin ${isDarkMode ? "text-indigo-400" : "text-indigo-600"}`}
          />
          <p
            className={`text-xl font-medium ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}
          >
            Loading Level 2 approvals...
          </p>
        </div>
      </div>
    );
  ...
  if (expenses.length === 0) {
    return (
      ...
      >
        <div className="text-center max-w-md">
          <h2
            className={`text-3xl font-bold mb-4 ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}
          >
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full space-y-8 lg:space-y-10">
        {/* Header */}
        <div
          className={`rounded-2xl border shadow-2xl w-full p-6 sm:p-8 lg:p-10 xl:p-12 ${
            isDarkMode
        ...
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
            <div>
              ...
              >
                Pending records for final review • {totalRecords} records
              </p>
            </div>

            <div
              className={`px-6 py-4 lg:py-6 rounded-2xl border shadow-lg min-w-[140px] lg:min-w-[180px] text-center ${
                isDarkMode
        ...
        {/* Table */}
        <div
          className={`overflow-x-auto rounded-2xl border shadow-2xl w-full ${
            isDarkMode
      ...
      {isModalOpen && selectedExpense && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 md:p-6 lg:p-8">
          <div
            className={`rounded-2xl shadow-2xl w-full max-w-full sm:max-w-3xl lg:max-w-5xl border max-h-[94vh] sm:max-h-[92vh] flex flex-col overflow-hidden ${
              isDarkMode
          ...
          >
            <div
              className={`p-5 sm:p-6 lg:p-8 flex justify-between items-center border-b shrink-0 ${
                isDarkMode
            ...
            <div
              className={`flex-1 overflow-y-auto px-5 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-10 space-y-6 sm:space-y-8 scrollbar-thin ${
                isDarkMode
                  ? "scrollbar-thumb-indigo-600/70 scrollbar-track-gray-950/40"
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 lg:gap-8">
                ...
                >
                  <p
                    className={`text-sm uppercase mb-2 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                  >
                    Bill No
                  </p>
                  <p
                    className={`text-xl sm:text-2xl font-bold break-all ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}
                  >
                    {selectedExpense.Office_Bill_No || "—"}
                  </p>
                </div>
                ...
                >
                  ...
                  <p
                    className={`text-2xl sm:text-3xl font-bold ${isDarkMode ? "text-emerald-400" : "text-emerald-600"}`}
                  >
                    ₹
                    {(
              ...
              <div className="space-y-3">
                <label
                  className={`block text-base sm:text-lg font-medium ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}
                >
                  Final Status <span className="text-red-400">*</span>
                </label>
                <select
                  value={formData.STATUS_3}
                  onChange={(e) =>
                    setFormData({ ...formData, STATUS_3: e.target.value })
                  }
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  value={formData.FINAL_AMOUNT_3}
                  onChange={(e) =>
                    setFormData({ ...formData, FINAL_AMOUNT_3: e.target.value })
                  }
              ...
              <div className="space-y-3">
                ...
                <textarea
                  value={formData.REMARK_3}
                  onChange={(e) =>
                    setFormData({ ...formData, REMARK_3: e.target.value })
                  }
                  rows={4}
            ...
            <div
              className={`shrink-0 border-t p-5 sm:p-6 lg:p-8 sticky bottom-0 z-10 ${
                isDarkMode
