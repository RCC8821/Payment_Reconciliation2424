...
function VRN_Approvel2() {
  ...
  const openModal = (expense) => {
    ...
    setFormData({
      STATUS_3: '',
      FINAL_AMOUNT_3: expense.PLANNED_3 || expense.Total_Amount || expense.Amount || '',
      REMARK_3: '',
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('Failed to submit: ' + (err?.data?.message || err.message || 'Unknown error'));
    }
  };
  ...
  return (
    <div className={`min-h-screen relative overflow-hidden py-8 px-4 sm:px-6 lg:px-8 xl:px-10 w-full ${
          ...
          <table className="w-full min-w-[1400px] border-collapse">
            ...
            <tbody className={`divide-y ${
              ...
              {expenses.map((item) => (
                ...
                >
                  <td className={`px-4 py-5 lg:px-6 lg:py-6 text-base ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{item.Timestamp || '-'}</td>
                  ...
                  <td className={`px-4 py-5 lg:px-6 lg:py-6 text-base ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>{item.APPROVAL_DOER || item.APPROVAL_DOER_2 || '-'}</td>
      ...
      {isModalOpen && selectedExpense && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 md:p-6 lg:p-8">
          <div className={`rounded-2xl shadow-2xl w-full max-w-full sm:max-w-3xl lg:max-w-5xl border max-h-[94vh] sm:max-h-[92vh] flex flex-col overflow-hidden ${
            isDarkMode 
