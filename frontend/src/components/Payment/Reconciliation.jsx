...
// // ─────────────────────────────────────────────────────────────────────────────
// // Email → allowed banks mapping
// // Add more entries here if other users also need bank restrictions
// // ─────────────────────────────────────────────────────────────────────────────
...
//   // ─────────────────────────────────────────────────────────────────────────
//   // uniqueBanks: filter based on logged-in user's email restriction
//   // If the email has a restriction → show only those banks
//   // Otherwise → show all banks (existing behaviour)
//   // ─────────────────────────────────────────────────────────────────────────
//   const uniqueBanks = useMemo(() => {
//     const allBanks = [...new Set(reconciliationData.map(item => item.bankDetails).filter(Boolean))];

//     // Check if this user has any bank restriction
//     const restrictedBanks = EMAIL_BANK_RESTRICTIONS[userEmail];
//     if (restrictedBanks && restrictedBanks.length > 0) {
//       // Show only the banks that exist in data AND are in the allowed list
//       return allBanks.filter(bank =>
//         restrictedBanks.some(allowed =>
//           bank.toLowerCase().trim() === allowed.toLowerCase().trim()
//         )
//       );
//     }

//     // No restriction — show all banks as before
//     return allBanks;
...
const Reconciliation = () => {
  ...
  // ── Filter effect (safe - no infinite loop) ───────────────────────────
  useEffect(() => {
    if (!selectedBank) {
  ...
  // ── Auto-calculate closing balance (safe - no infinite loop) ──────────
  useEffect(() => {
    if (
      !isModalOpen ||
  ...
  const handleSaveReconciliation = async (e) => {
    ...
    // firmName = contractorName from selected item
    const payload = {
      paymentDetails:                (selectedItem?.paymentDetails || "").trim(),
      bankDetails:                   (selectedItem?.bankDetails    || "").trim(),
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          className={`absolute -top-20 -left-20 w-[500px] h-[500px] rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-pulse-slow ${
      ...
      <div className="relative z-10 space-y-8">
        ...
        <div
          className={`rounded-2xl border shadow-2xl w-full p-6 sm:p-8 lg:p-10 ${
            isDarkMode
              ? "bg-black/70 border-indigo-700/60"
              : "bg-white/90 border-indigo-200/80"
          }`}
        >
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
              <h1
                className={`text-3xl sm:text-4xl lg:text-5xl font-bold bg-gradient-to-r bg-clip-text text-transparent flex items-center gap-3 ${
                  isDarkMode
        ...
        <div
          className={`rounded-xl border p-4 shadow-lg ${
            isDarkMode
              ? "bg-black/50 border-indigo-700/50"
              : "bg-white/80 border-indigo-200/70"
          }`}
        >
          <select
            value={selectedBank}
              ...
              <div className="space-y-6">
                <div className="relative">
                  <input
                    ...
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className={`w-full pl-11 pr-4 py-3 rounded-xl border-2 focus:outline-none focus:ring-2 transition-all ${
                      isDarkMode
                        ? "bg-gray-900/60 border-gray-700 hover:border-indigo-500 text-white placeholder-gray-500 focus:ring-indigo-500"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      placeholder="Add remark if needed..."
                      className={`w-full px-5 py-4 rounded-xl resize-none border focus:outline-none focus:ring-2 ${
                        isDarkMode
                          ? "bg-gray-900/70 border-gray-700 text-white focus:ring-indigo-500"
                          : "bg-white border-gray-300 text-gray-900 focus:ring-indigo-400"
                      }`}
      ...
      {/* Global CSS - plain style tag (no jsx attribute) */}
      <style>{`
        @keyframes float {
          0%, 100% { transform: translate(0, 0); }
        ...
        ::-webkit-scrollbar-thumb {
          background: ${isDarkMode ? "rgba(99,102,241,0.5)" : "rgba(99,102,241,0.3)"};
          border-radius: 5px;
        }
        ::-webkit-scrollbar-thumb:hover {
          background: ${isDarkMode ? "rgba(99,102,241,0.7)" : "rgba(99,102,241,0.5)"};
        }
      `}</style>
    </div>
  );
