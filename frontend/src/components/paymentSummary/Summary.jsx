...
const Outstanding = ({ isDarkMode }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (
      filters.vendorNames.length > 0 &&
      !filters.vendorNames.includes("All")
    ) {
  ...
  const filterOptions = useMemo(() => {
    ...
    const vendors = [
      ...new Set(current.map((t) => t.vendorName).filter(Boolean)),
    ].sort();
    ...
    if (
      ...
      current = current.filter((t) =>
        filters.vendorNames.includes(t.vendorName),
      );
    }

    const heads = [
      ...new Set(current.map((t) => t.expHead).filter(Boolean)),
    ].sort();
    ...
    const billNumbers = [
      ...new Set(current.map((t) => t.billNo).filter(Boolean)),
    ].sort();

    return {
      siteNames: [
        "All",
        ...[...new Set(rawData.map((t) => t.siteName).filter(Boolean))].sort(),
      ],
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 lg:gap-6">
        <div
          className={`rounded-xl border shadow-lg p-6 text-center transition-all hover:shadow-xl ${isDarkMode ? "bg-gradient-to-br from-rose-950/40 to-rose-900/30 border-rose-800/50" : "bg-gradient-to-br from-rose-50 to-rose-100 border-rose-200"}`}
        >
          <p
            className={`text-sm font-medium uppercase tracking-wide mb-2 ${isDarkMode ? "text-rose-300" : "text-rose-700"}`}
          >
            Total Billed
          </p>
          <p
            className={`text-3xl lg:text-4xl font-black ${isDarkMode ? "text-rose-400" : "text-rose-700"}`}
          >
        ...
        <div
          className={`rounded-xl border shadow-lg p-6 text-center transition-all hover:shadow-xl ${isDarkMode ? "bg-gradient-to-br from-emerald-950/40 to-emerald-900/30 border-emerald-800/50" : "bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200"}`}
        >
          <p
            className={`text-sm font-medium uppercase tracking-wide mb-2 ${isDarkMode ? "text-emerald-300" : "text-emerald-700"}`}
          >
            Total Paid
          </p>
          <p
            className={`text-3xl lg:text-4xl font-black ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}
          >
        ...
        <div
          className={`rounded-xl border shadow-lg p-6 text-center transition-all hover:shadow-xl ${isDarkMode ? "bg-gradient-to-br from-indigo-950/40 to-purple-950/30 border-indigo-800/50" : "bg-gradient-to-br from-indigo-50 to-purple-100 border-indigo-200"}`}
        >
          <p
            className={`text-sm font-medium uppercase tracking-wide mb-2 ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}
          >
            Outstanding
          </p>
          <p
      ...
      {/* Filters Section */}
      <div
        className={`rounded-xl border shadow-lg p-6 lg:p-8 ${isDarkMode ? "bg-gray-900/40 border-gray-700/60" : "bg-white border-gray-200/80"}`}
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h3
            className={`text-xl lg:text-2xl font-bold flex items-center gap-3 ${isDarkMode ? "text-white" : "text-gray-900"}`}
          >
            <Search
            ...
            {activeFiltersCount > 0 && (
              <span
                className={`ml-3 px-3 py-1 text-xs font-semibold rounded-full ${isDarkMode ? "bg-indigo-900/80 text-indigo-200" : "bg-indigo-100 text-indigo-800"}`}
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          ...
          <MultiSelectFilter
            ...
            onChange={(vals) =>
              setFilters((prev) => ({ ...prev, billNos: vals }))
            }
      ...
      <div
        className={`rounded-xl border shadow-lg overflow-hidden ${isDarkMode ? "bg-gray-900/30 border-gray-700/50" : "bg-white border-gray-200/70"}`}
      >
        <div
          className={`px-6 py-5 border-b font-semibold text-lg flex items-center justify-between ${isDarkMode ? "bg-gradient-to-r from-indigo-950/70 to-purple-950/50 text-indigo-200 border-gray-700/60" : "bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-800 border-indigo-200/40"}`}
        >
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[1200px]">
                <thead
                  className={`${isDarkMode ? "bg-gray-800/40" : "bg-gray-100/80"}`}
                >
                  <tr>
                    <th
                      className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                    >
                      Date
                    </th>
                    <th
                    ...
                    <th
                      className={`px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider w-32 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                    >
                      Bill No
                    </th>{" "}
                    {/* ← fixed width */}
                    <th
                      className={`px-6 py-4 text-right text-xs font-semibold uppercase tracking-wider ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}
                    >
                      Net Amount
                    </th>
                    <th
                ...
                <tbody
                  className={`divide-y ${isDarkMode ? "divide-gray-800/40" : "divide-gray-200/40"}`}
                >
                  {displayedData.map((item, index) => (
                    ...
                    >
                      ...
                      <td
                        className={`px-6 py-4 text-sm font-medium ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}
                      >
                        {item.siteName}
                      </td>
                      <td
                      ...
                      <td
                        className={`px-6 py-4 text-sm truncate max-w-[120px] ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
                      >
                        {item.billNo}
                      </td>
                      <td
                        className={`px-6 py-4 text-right text-sm font-medium ${isDarkMode ? "text-gray-300" : "text-gray-800"}`}
                      >
                        ₹
                      ...
                      <td
                        className={`px-6 py-4 text-right text-sm font-medium ${isDarkMode ? "text-emerald-400" : "text-emerald-700"}`}
                      >
                        ₹
                      ...
                      <td
                        className={`px-6 py-4 text-right text-lg font-bold ${item.balance > 0 ? "text-rose-500" : "text-emerald-500"}`}
                      >
                        ₹
            ...
            {filteredData.length > 10 && (
              <div
                className={`px-6 py-5 border-t text-center ${isDarkMode ? "bg-gray-900/30 border-gray-700/50" : "bg-gray-50 border-gray-200"}`}
              >
                <button
        ...
        <div
          className={`px-6 py-5 border-t font-bold text-right text-lg flex justify-between items-center ${isDarkMode ? "bg-gray-900/40 border-gray-700/60 text-indigo-300" : "bg-gray-50 border-gray-200 text-indigo-800"}`}
        >
          <span>Total Outstanding (filtered):</span>
          <span
            className={
              totalOutstanding >= 0 ? "text-rose-500" : "text-emerald-500"
            }
          >
            ₹
            {Math.abs(totalOutstanding).toLocaleString("en-IN", {
