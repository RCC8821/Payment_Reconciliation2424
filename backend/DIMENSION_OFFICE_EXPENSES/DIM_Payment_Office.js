...
router.get('/Get-DIM-Payment', async (req, res) => {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: 'DIMENSION_OFFICE_PAYMANT!A7:AB',   
    });
    ...
    .map(row => ({
      ...
      PLANNED_4   : (row[26] || '').toString().trim(),   // K (most common position for amount)
    }));
...
router.post('/update-DIM-OFFICE-Payment', async (req, res) => {
  ...
  try {
    ...
    const getResponse = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID_OFFICE_EXPENSES,
      range: 'DIMENSION_OFFICE_PAYMANT!B7:B', // adjust sheet name / range if needed
    });
    ...
    const updates = [
      { range: `DIMENSION_OFFICE_PAYMANT!AC${sheetRowNumber}`, values: [[STATUS_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AE${sheetRowNumber}`, values: [[TOTAL_PAID_AMOUNT_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AF${sheetRowNumber}`, values: [[BASIC_AMOUNT_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AG${sheetRowNumber}`, values: [[CGST_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AH${sheetRowNumber}`, values: [[SGST_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AI${sheetRowNumber}`, values: [[NET_AMOUNT_4 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AJ${sheetRowNumber}`, values: [[PAYMENT_MODE_17 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AK${sheetRowNumber}`, values: [[BANK_DETAILS_17 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AL${sheetRowNumber}`, values: [[PAYMENT_DETAILS_17 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AM${sheetRowNumber}`, values: [[PAYMENT_DATE_17 || '']] },
      { range: `DIMENSION_OFFICE_PAYMANT!AN${sheetRowNumber}`, values: [[Remark_Blank || '']] },
    ];
